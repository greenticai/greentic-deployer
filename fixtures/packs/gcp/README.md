...
This fixture represents the `greentic.deploy.gcp` deployment pack for `PR-04`.

It is a provider-specific extension pack built on the shared Terraform-based
deployment layout, but versioned as a GCP pack rather than a generic
multicloud pack.
...
- the pack input contract is GCP-specific and no longer asks the caller to
  choose a cloud
