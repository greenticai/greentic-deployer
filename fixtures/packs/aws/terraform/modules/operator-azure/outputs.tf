...
output "admin_client_cert_secret_ref" {
  value = local.can_manage_key_vault_secrets ? azurerm_key_vault_secret.admin_client_cert[0].versionless_id : (
    local.secret_prefix != null ? "${local.secret_prefix}/secrets/${local.admin_client_cert_secret_name}" : null
  )
}

output "admin_client_key_secret_ref" {
  value = local.can_manage_key_vault_secrets ? azurerm_key_vault_secret.admin_client_key[0].versionless_id : (
    local.secret_prefix != null ? "${local.secret_prefix}/secrets/${local.admin_client_key_secret_name}" : null
  )
}

output "admin_relay_token_secret_ref" {
  value = local.can_manage_key_vault_secrets ? azurerm_key_vault_secret.admin_relay_token[0].versionless_id : (
    local.secret_prefix != null ? "${local.secret_prefix}/secrets/${local.admin_relay_token_secret_name}" : null
  )
}
