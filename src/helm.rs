...
/// Library-facing request for the explicit Helm adapter surface.
#[derive(Debug, Clone)]
...
pub fn resolve_config(request: HelmRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_helm_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::K8s || config.strategy != "helm" {
        return Err(DeployerError::Config(format!(
            "helm adapter requires provider=k8s strategy=helm, got provider={} strategy={}",
...
pub async fn run(request: HelmRequest) -> Result<multi_target::OperationResult> {
