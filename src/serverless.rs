...
/// Library-facing request for the explicit serverless adapter surface.
#[derive(Debug, Clone)]
...
impl ServerlessRequest {
    ...
    pub fn into_deployer_request(self) -> DeployerRequest {
        DeployerRequest {
            ...
            strategy: "serverless-container".to_string(),
            tenant: self.tenant,
...
pub fn resolve_config(request: ServerlessRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_serverless_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::Generic || config.strategy != "serverless-container" {
        return Err(DeployerError::Config(format!(
            "serverless adapter requires provider=generic strategy=serverless-container, got provider={} strategy={}",
...
pub async fn run(request: ServerlessRequest) -> Result<multi_target::OperationResult> {
...
pub async fn run_config(config: DeployerConfig) -> Result<multi_target::OperationResult> {
    ensure_serverless_config(&config)?;
...
mod tests {
    ...
    #[test]
    fn serverless_request_defaults_to_generic_serverless_target() {
        let request =
            ServerlessRequest::new(DeployerCapability::Plan, "acme", PathBuf::from("pack-dir"))
                .into_deployer_request();

        assert_eq!(request.provider, Provider::Generic);
        assert_eq!(request.strategy, "serverless-container");
