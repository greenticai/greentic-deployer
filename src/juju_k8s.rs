...
pub fn resolve_config(request: JujuK8sRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_juju_k8s_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::K8s || config.strategy != "juju-k8s" {
        return Err(DeployerError::Config(format!(
            "juju-k8s adapter requires provider=k8s strategy=juju-k8s, got provider={} strategy={}",
...
pub async fn run(request: JujuK8sRequest) -> Result<multi_target::OperationResult> {
...
mod tests {
    ...
    #[test]
    fn juju_k8s_request_defaults_to_k8s_juju_k8s_target() {
        let request =
            JujuK8sRequest::new(DeployerCapability::Plan, "acme", PathBuf::from("pack-dir"))
                .into_deployer_request();

        assert_eq!(request.provider, Provider::K8s);
        assert_eq!(request.strategy, "juju-k8s");
    }
}
