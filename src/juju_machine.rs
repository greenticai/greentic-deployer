...
impl JujuMachineRequest {
    ...
    pub fn into_deployer_request(self) -> DeployerRequest {
        DeployerRequest {
            ...
            strategy: "juju-machine".to_string(),
            tenant: self.tenant,
...
pub fn resolve_config(request: JujuMachineRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_juju_machine_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::Local || config.strategy != "juju-machine" {
        return Err(DeployerError::Config(format!(
            "juju-machine adapter requires provider=local strategy=juju-machine, got provider={} strategy={}",
...
pub async fn run(request: JujuMachineRequest) -> Result<multi_target::OperationResult> {
...
pub async fn run_config(config: DeployerConfig) -> Result<multi_target::OperationResult> {
    ensure_juju_machine_config(&config)?;
    multi_target::run(config).await
}

pub async fn run_with_plan(
    request: JujuMachineRequest,
    plan: PlanContext,
...
mod tests {
    ...
    #[test]
    fn juju_machine_request_defaults_to_local_juju_machine_target() {
        let request =
            JujuMachineRequest::new(DeployerCapability::Plan, "acme", PathBuf::from("pack-dir"))
                .into_deployer_request();

        assert_eq!(request.provider, Provider::Local);
        assert_eq!(request.strategy, "juju-machine");
    }
}
