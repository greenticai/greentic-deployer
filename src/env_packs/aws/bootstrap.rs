...
- **ALB listener mutation** (`elasticloadbalancing:ModifyListener`,
  `DescribeRules`, `CreateRule`, `ModifyRule`) — `ModifyListener` writes the
  listener's default action; the rule verbs write per-deployment listener
  rules. Replace `<REPLACE_WITH_ALB_LISTENER_ARNS>` with your listener ARN.
