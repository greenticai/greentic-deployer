...
/// Library-facing request for the explicit operator adapter surface.
#[derive(Debug, Clone)]
...
pub fn resolve_config(request: OperatorRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_operator_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::K8s || config.strategy != "operator" {
        return Err(DeployerError::Config(format!(
            "operator adapter requires provider=k8s strategy=operator, got provider={} strategy={}",
...
pub async fn run(request: OperatorRequest) -> Result<multi_target::OperationResult> {
...
mod tests {
    ...
    #[test]
    fn operator_request_defaults_to_k8s_operator_target() {
        let request =
            OperatorRequest::new(DeployerCapability::Plan, "acme", PathBuf::from("pack-dir"))
                .into_deployer_request();

        assert_eq!(request.provider, Provider::K8s);
        assert_eq!(request.strategy, "operator");
