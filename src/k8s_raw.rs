...
/// Library-facing request for the explicit k8s raw-manifests adapter surface.
#[derive(Debug, Clone)]
pub struct K8sRawRequest {
...
impl K8sRawRequest {
    ...
    pub fn into_deployer_request(self) -> DeployerRequest {
        DeployerRequest {
            ...
            strategy: "raw-manifests".to_string(),
            tenant: self.tenant,
...
pub fn resolve_config(request: K8sRawRequest) -> Result<DeployerConfig> {
    DeployerConfig::resolve(request.into_deployer_request())
}

pub fn ensure_k8s_raw_config(config: &DeployerConfig) -> Result<()> {
    if config.provider != Provider::K8s || config.strategy != "raw-manifests" {
        return Err(DeployerError::Config(format!(
            "k8s_raw adapter requires provider=k8s strategy=raw-manifests, got provider={} strategy={}",
...
pub async fn run(request: K8sRawRequest) -> Result<multi_target::OperationResult> {
...
mod tests {
    ...
    #[test]
    fn k8s_raw_request_defaults_to_k8s_raw_manifests_target() {
        let request =
            K8sRawRequest::new(DeployerCapability::Plan, "acme", PathBuf::from("pack-dir"))
                .into_deployer_request();

        assert_eq!(request.provider, Provider::K8s);
        assert_eq!(request.strategy, "raw-manifests");
