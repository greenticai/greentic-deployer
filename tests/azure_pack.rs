...
fn fixture_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/packs/azure")
}
...
#[test]
fn azure_contract_references_existing_assets() {
...
#[test]
fn azure_contract_uses_azure_specific_flow_ids() {
    let root = fixture_root();
    let contract: DeployerContractV1 =
...
#[test]
fn azure_examples_validate_against_pack_schemas() {
    let root = fixture_root();
...
#[test]
fn azure_root_contains_required_files_and_modules() {
...
#[test]
fn azure_fixture_omits_other_cloud_modules() {
    let root = fixture_root().join("terraform");
    for path in ["modules/operator-azure", "modules/operator-gcp"] {
        assert!(
            !root.join(path).exists(),
            "azure fixture should not ship {}; the Azure module lives at modules/operator/",
            path
        );
    }
}
...
#[test]
fn azure_files_are_deterministic_and_secret_free() {
    let root = fixture_root().join("terraform");
    let combined = [
        "main.tf",
        "variables.tf",
...
#[test]
fn azure_providers_pin_azurerm_backend_and_omit_other_cloud_providers() {
    let providers =
...
#[test]
fn azure_main_tf_only_wires_azure_operator_module() {
...
#[test]
fn azure_outputs_resolve_directly_against_azure_operator_module() {
    let outputs =
        fs::read_to_string(fixture_root().join("terraform/outputs.tf")).expect("read outputs");
    assert!(outputs.contains("module.operator_azure.operator_endpoint"));
    assert!(outputs.contains("module.operator_azure.admin_ca_secret_ref"));
    assert!(outputs.contains("module.operator_azure.admin_relay_token_secret_ref"));
    assert!(!outputs.contains("module.operator_aws"));
    assert!(!outputs.contains("module.operator_gcp"));
}

#[test]
fn azure_generate_output_advertises_azure_only_layout() {
    ...
    assert_eq!(supported[0].as_str(), Some("azure"));
...
#[test]
fn azure_plan_output_declares_terraform_commands_and_variables() {
...
fn azure_module_materializes_key_vault_and_runtime_resources() {
    ...
    assert!(variables_tf.contains("variable \"azure_key_vault_id\""));
    assert!(variables_tf.contains("variable \"azure_key_vault_uri\""));
    assert!(variables_tf.contains("variable \"azure_location\""));
}
