...
#[test]
fn gcp_generate_cli_renders_json_output() {
    let dir = tempfile::tempdir().expect("tempdir");
    let provider_pack = dir.path().join("provider.gtpack");
    build_provider_gtpack("terraform", &provider_pack, "greentic.deploy.gcp");
    ...
    assert!(stdout.contains("\"provider\": \"gcp\""));
    assert!(stdout.contains("\"strategy\": \"iac-only\""));
}

#[test]
fn gcp_status_cli_renders_json_output() {
    let dir = tempfile::tempdir().expect("tempdir");
    let provider_pack = dir.path().join("provider.gtpack");
    build_provider_gtpack("terraform", &provider_pack, "greentic.deploy.gcp");
    let pack = example_pack_path();
...
#[test]
fn gcp_apply_execute_cli_runs_local_terraform_scaffold() {
    let dir = tempfile::tempdir().expect("tempdir");
    let provider_pack = dir.path().join("provider.gtpack");
    build_provider_gtpack("terraform", &provider_pack, "greentic.deploy.gcp");
